 ...
 */
public class TLRequestChatlistsGetChatlistUpdates extends TLMethod<TLChatlistUpdates> {
    public static final int CONSTRUCTOR_ID = 0x0;

    protected TLInputChatlistDialogFilter chatlist;

    private final String _constructor = "chatlists.getChatlistUpdates#0";

    public TLRequestChatlistsGetChatlistUpdates() {
    }

    public TLRequestChatlistsGetChatlistUpdates(TLInputChatlistDialogFilter chatlist) {
        this.chatlist = chatlist;
    }

    @Override
    @SuppressWarnings({"unchecked", "SimplifiableConditionalExpression"})
    public TLChatlistUpdates deserializeResponse(InputStream stream, TLContext context) throws IOException {
        final TLObject response = readTLObject(stream, context);
        if (response == null) {
            throw new IOException("Unable to parse response");
        }
        if (!(response instanceof TLChatlistUpdates)) {
            throw new IOException("Incorrect response type, expected " + getClass().getCanonicalName() + ", found " + response.getClass().getCanonicalName());
        }
        return (TLChatlistUpdates) response;
    }
