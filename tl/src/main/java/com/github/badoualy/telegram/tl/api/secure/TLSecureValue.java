...
public class TLSecureValue extends TLObject {
    ...
    public TLSecureValue(TLAbsSecureValueType type, TLSecureData data, TLAbsSecureFile frontSide,
                         TLAbsSecureFile reverseSide, TLAbsSecureFile selfie, TLVector<TLAbsSecureFile> translation,
                         TLVector<TLAbsSecureFile> files, TLAbsSecurePlainData plainData, TLBytes hash) {
        this.type = type;
        this.data = data;
    ...
    public void serializeBody(OutputStream stream) throws IOException {
        ...
        if ((flags & 1) != 0) {
            if (data == null) {
                throwNullFieldException("data", flags);
            }
            writeTLObject(data, stream);
        }
        if ((flags & 2) != 0) {
            if (frontSide == null) {
                throwNullFieldException("frontSide", flags);
            }
            writeTLObject(frontSide, stream);
        }
        if ((flags & 4) != 0) {
            if (reverseSide == null) {
                throwNullFieldException("reverseSide", flags);
            }
            writeTLObject(reverseSide, stream);
        }
        if ((flags & 8) != 0) {
            if (selfie == null) {
                throwNullFieldException("selfie", flags);
            }
            writeTLObject(selfie, stream);
        }
        if ((flags & 64) != 0) {
            if (translation == null) {
                throwNullFieldException("translation", flags);
            }
            writeTLVector(translation, stream);
        }
        if ((flags & 16) != 0) {
            if (files == null) {
                throwNullFieldException("files", flags);
            }
            writeTLVector(files, stream);
        }
        if ((flags & 32) != 0) {
            if (plainData == null) {
                throwNullFieldException("plainData", flags);
            }
